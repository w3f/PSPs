...
All parameters in the specified JSON-RPC methods are REQUIRED, unless explicitly mentioned otherwise (OPTIONAL).
...
* `OBJECT` - (MUST)
  * `STRING`: `VALUE` - (MAY) Property name and value
...
* `STRING` - (MUST) SCALE encoded, concatenated keys represented as hex.

#### Returns

* `BOOL` - (MUST) Returns `true` if all private keys could be found, `false` if otherwise.
...
* `BOOL` - (MUST) Returns `true` if a private key could be found, `false` if otherwise.
...
* `ARRAY` - (MUST)
  * `STRING` - (MAY) Hex encoded hash of extrinsic that was removed.
...
* `STRING` - (MAY) Hex encoded value indicating the 'n-th' block in the chain.
* `U32` - (MAY) Value indicating the 'n-th' block in the chain.
* `ARRAY` - (MAY) Multiple values, where each value can be one of the following types:
  * `STRING` - (MAY) Hex encoded value indicating the 'n-th' block in the chain.
  * `U32` - (MAY) Value indicating the 'n-th' block in the chain.
