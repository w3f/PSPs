...
The implementation of this specification should disable the JSON-RPC API by default. When enabled, the API should only accept requests from localhost. Additionally, two options should be available and have to be explicitly enabled: an option to expose *SAFE* functions on every interface, and another option to expose *UNSAFE* functions on every interface.
...
* *Pool*
  * Occurence: some issue with the client.
  * RPC code: .
  * RPC message: "Client error: ${details}"
    * *${details}*: details of the error message.
* *Pool*
  * Occurence: transaction pool error.
  * RPC code: .
  * RPC message: "Transaction pool error: ${details}"
    * *${details}*: details of the error message.
* *Verification*
  * Occurence: verification error.
  * RPC code: .
  * RPC message: "Extrinsic verification error: ${details}"
    * *${details}*: details of the error message.
* *BadFormat*
  * Occurence: incorrect extrinsic format.
  * RPC code: .
  * RPC message: "Invalid extrinsic format: ${details}"
    * *${details}*: details of the error message.
* *BadSeedPhrase*
  * Occurence: incorrect seed phrase.
  * RPC code: .
  * RPC message: "Invalid seed phrase/SURI"
* *BadKeyType*
  * Occurence: key type ID has an unknown format.
  * RPC code: .
  * RPC message: "Invalid key type ID format (should be of length four)"
* *UnsupportedKeyType*
  * Occurence: key type ID has some unsupported crypto.
  * RPC code: .
  * RPC message: "The crypto of key type ID is unknown"
* *KeyStoreUnavailable*
  * Occurence: some issue with the key store.
  * RPC code: .
  * RPC message: "The key store is unavailable"
* *InvalidSessionKeys*
  * Occurence: invalid session keys encoding.
  * RPC code: .
  * RPC message: "Session keys are not encoded correctly"
...
* [*BadKeyType*](#author-errors)
* [*KeyStoreUnavailable*](#author-errors)
...
* [*Client*](#author-errors)
* [*InvalidSessionKeys*](#author-errors)
...
* *InvalidBlockRange*
  * Occurence: provided block range couldn't be resolved to a list of blocks.
  * RPC code: TODO
  * RPC message: "Cannot resolve a block range ['\${from}' ... '${to}]. ${details}"
    * *${from}*: beginning of the block range.
    * *${to}*: end of the block range.
    * *${details}*: details of the error message.
* *InvalidCount*
  * Occurrence: provided count exceeds maximum value of 1'000.
  * RPC code: 4002
  * RPC message: "*count exceeds maximum value. value: ${value}, max: 1000*"
    * *${value}*: the provided count
...
* [*InvalidBlockRange*](#state-errors)
...
TODO: Not implemented in final build.
