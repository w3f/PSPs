...
The implementation of this specification must disable the JSON-RPC API by default and should offer four different flags for enabling specific parts of the API. Two flags for exposing safe and unsafe PRC to localhost only, and two flags for exposing safe and unsafe RPC to all interfaces.

| API / Network | Localhost            | All interfaces            |
| ------------- | -------------------- | ------------------------- |
| Safe API      | `--local-rpc`        | `--public-rpc`            |
| Unsafe API    | `--local-unsafe-rpc` | `--public-unsafe-rpc`     |
