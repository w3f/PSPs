...
`STRING` - (MUST) The node's version.
...
* `OBJECT`
  * `STRING`: `VALUE` - (MUST) Property name and value
...
* `OBJECT`
  * `"isSyncing"`: `BOOL` - (MUST) Whether the node is syncing.
  * `"peers"`: `U32` - (MUST) Number of connected peers.
  * `"shouldHavePeers"`: `BOOL` - (MUST) Should this node have any peers. Might be false for local chains or when running without discovery.
...
* `ARRAY`
  * `OBJECT`
    * `"PeerId"`: `STRING` - (MUST) Peer ID.
    * `"roles"`: `STRING` - (MUST) Roles of the node. One of the following values is possible:
      * `"NONE"` - (MUST) No network.
      * `"FULL"` - (MUST) Full node, does not participate in consensus.
      * `"LIGHT"` - (MUST) Light client node.
      * `"AUTHORITY"` - (MUST) Acts as an authority.
    * `"protocolVersion"`: `U32` - (MUST) Protocol version.
    * `"bestHash"`: `STRING` - (MUST) Peer best block hash.
    * `"bestNumber"`: `U64` - (MUST) Peer best block number.
...
* `STRING` - (MUST) Multiaddr to be added.
...
* `STRING` - (MUST) Peer ID to be removed.
...
* `ARRAY`
  * `STRING` - (MUST) One of the following values is possible:
    * `"Full"` - (MUST) The node is a full node.
    * `"LightClient"` - (MUST) The node is a a light client.
    * `"Authority"` - (MUST) The node is an authority.
    * `"UnknownRole` - (MUST) An unknown role followed by an arbitrary byte value. *TODO*: define additional byte value.
...
* `STRING` - (MUST) The address of the account.

#### Returns

* `U64` - (MUST) The nonce of the account.
...
* `"babe"` - (MUST) Babe functionality.
* `"gran"` - (MUST) GRANDPA functionality.
* `"acco"` - (MUST) Controlling an account.
* `"aura"` - (MUST) Aura functionality.
* `"imon"` - (MUST) *ImOnline* functionality.
* `"audi"` - (MUST) Authority discovery functionality.
* `"dumy"` - (MUST) Useful for testing.
...
* `STRING` - (MUST) A SCALE encoded extrinsic represented as hex.
...
* `STRING` - (MUST) [Key type](#Key-types).
* `STRING` - (MUST) Hex encoded seed.
* `STRING` - (MUST) Hex encoded public.
...
* `STRING` - (MUST) SCALE encoded keys represented as hex.
...
* `BOOL` - (MUST) Returns `true` if all private keys could be found.
...
* `STRING` - (MUST) Hex-encoded public key.
* `STRING` - (MUST) [Key type](#Key-types).

#### Returns

* `BOOL` - (MUST) Returns `true` if a private key could be found.
...
* `ARRAY`
  * `STRING` - (MAY) SCALE encoded extrinsic represented as hex.
...
* `STRING` - (MUST) SCALE encoded array of extrinsics or its hash represented as hex.

#### Returns

* `ARRAY`
  * `STRING` - (MUST) Hex encoded hash of extrinsic that was removed.
...
* `STRING` - (MAY) Hex encoded block hash.

#### Returns

* `OBJECT` - (MAY).
  * `"parentHash"`: `STRING` - (MUST) Hex encoded parent hash.
  * `"number"`: `STRING` - (MUST) Hex encoded block number.
  * `"stateRoot"`: `STRING` - (MUST) Hex encoded state trie merkle root.
  * `"extrinsicsRoot"`: `STRING` - (MUST) Hex encoded merkle root of the extrinsics.
  * `"digest"`: `OBJECT` - (MUST) Chain-specific digest of data useful for light clients or auxiliary data.
    * `"log"`: `ARRAY` - (MUST) A list of logs in the digest.
      * `OBJECT` - (MUST) A *DigestItem* object. Contains varying data types. *TODO*: define those.
...
* `OBJECT` - (MAY)
  * `"block"`: `OBJECT` - (MUST)
    * `"extrinsics"`: `ARRAY` - (MUST)
      * `STRING` - (MUST) Hex encoded extrinsic.
    * `"header"`: `OBJECT`
      * `"parentHash"`: `STRING` - (MUST) Hex encoded parent hash.
      * `"number"`: `STRING` - (MUST) Hex encoded block number.
      * `"stateRoot"`: `STRING` - (MUST) Hex encoded state trie merkle root.
      ...
      * `"digest"`: `OBJECT` - (MUST) Chain-specific digest of data useful for light clients or auxiliary data.
        * `"log"`: `ARRAY` - (MUST) A list of logs in the digest.
          * `OBJECT` - (MUST) A *DigestItem* object. Contains varying data types. *TODO*: define those.
  * `"justification"`: `STRING` - (MAY) The hex encoded justification.
...
* (optional) One of the following types is allowed:
  * `STRING` - (MUST) Hex encoded value indicating the 'n-th' block in the chain.
  * `U32` - (MUST) Value indicating the 'n-th' block in the chain.
  * `ARRAY` - (MUST) Multiple values, where each value can be one of the following types:
    * `STRING` - (MUST) Hex encoded value indicating the 'n-th' block in the chain.
    * `U32` - (MUST) Value indicating the 'n-th' block in the chain.
...
* (optional) One of the following types gets returned, depending on the provided parameters:
  * `STRING` - (MUST) Hex encoded block hash.
  * `ARRAY`
    * `STRING` - (MUST) Hex encoded block hash
...
* `STRING` - (MUST) Hex encoded hash of the last finalized block.
...
* `STRING` - (MUST) [Storage kind](#Storage-kinds).
* `STRING` - (MUST) Hex encoded key.
* `STRING` - (MUST) Hex encoded value.
...
* `STRING` - (MUST) Hex encoded prefix.
* `STRING` - (MAY) Hex encoded block hash.
...
* `ARRAY`
  * `OBJECT`
    * `STRING` - (MUST) Hex encoded storage key
    * `STRING` - (MUST) Hex encoded storage data
...
* `STRING` - (MAY) Hex encoded prefix.
* `U32` - (MUST) Amount of keys to be return.
* `STRING` - (MAY) Hex encoded storage key after which the next keys should be returned in lexicographic order.
* `STRING` - (MAY) Hex encoded block hash.

#### Returns

* `ARRAY`
  * `STRING` - (MUST) Storage key
...
* `STRING` - (MAY) Hex encoded storage data.
...
* `STRING` - (MAY) Hex encoded hash of the storage entry.
...
* `U64` - (MAY) The size of the storage entry in bytes.
...
* `STRING` - (MUST) Hex encoded storage key.
* `STRING` - (MUST) [Child info](#child-info).
* `U32` - (MUST) [Child type](#child-type).
* `STRING` - (MUST) Prefix.
* `STRING` - (MAY) Hex encoded block hash.
...
* `STRING` - (MAY) Hex encoded hash of the child storage.
...
* `OBJECT`
  * `"specName"`: `STRING` - (MUST) Name of the runtime.
  * `"implName"`: `STRING` - (MUST) Name of the implementation of the specification.
  * `"authoringVersion"`: `U32` - (MUST) Version of the authorship interface.
  * `"specVersion"`: `U32` - (MUST) Version of the runtime specification.
  * `"implVersion"`: `U32` - (MUST) Version of the implementation of the specification.
  * `"apis"`: `ARRAY` - (MUST) List of supported API features along with their version.
    * `OBJECT`
      * `STRING` - (MUST) Hex encoded feature name
      * `U32` - (MUST) Version of the feature
...
* `ARRAY`
  * `STRING` - (MUST) Hex encoded storage key.
* `STRING` - (MUST) Hex encoded block hash.
* `STRING` - (MAY) Hex encoded hash. *TODO: which hash?
