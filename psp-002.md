...
This standard describes the JSON-RPC interfaces that Polkadot node implementation should provide, including each function calls parameter types, behavior and possible return values.

## Motivation

This standard should serve as common ground for API endpoints in order for external applications being able to reuse existing functionality on various, different implementations of the Polkadot node. Avoiding disparity among node implementations will simplify the development of such applications.
